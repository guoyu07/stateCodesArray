...
class statesClass
{
    ...
    public function findStateCodesByName($stateName){       //whole name or part thereof
        global $theName;
        $theName = $stateName;

        $result = array_filter($this->states, "testName");
        $result['XX']=$result['xx']="";
        foreach($result as $mainCode=>$value){
            if($mainCode{0}=='U' && $mainCode{1}=='S'){
                $result['fullname']=$value;
                $result['XX']=$result['xx']=$mainCode{3} . $mainCode{4} . "";
            }
            break;
        }
        return $result;                                     //returns an array with only the codes that match
                                                            //$result['xx'] will contain the 2 digit code
    }

    public function findStateCodeByName($stateName){       //whole name or part thereof
        $codes = $this->findStateCodesByName($stateName);
        if($codes['XX']!=""){
            return $codes['XX'];
        }
        foreach($codes as $mainCode=>$value){
            return $mainCode;
            break;
        }
        return;
    ...
    public function findStateFullNameByPartial($stateName){       //whole name or part thereof
        $codes = $this->findStateCodesByName($stateName);                //take the first wild ass guess at the name
        if($codes['XX']!=""){
    ...
    public function cleanCode($code){     //strips double spaces and []()tabs and linefeeds
        ...
        $code=str_replace('[',"",$code);
        $code=str_replace(']',"",$code);
        $code=str_replace('=',"",$code);  //stop potential sql injection through the _GET[]
        $code=str_replace('>',"",$code);
        $code=str_replace('>',"",$code);
        $code=str_replace('/',"",$code);
        $code=str_replace("\\","",$code);
        $code=str_replace('exec',"",$code);
